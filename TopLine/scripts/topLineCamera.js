(function($, undefined) {
	document.addEventListener("deviceready", onDeviceReady, false);
 
	function id(element) {
		return document.getElementById(element);
	}

	function onDeviceReady() {
		cameraApp = new cameraApp();
		cameraApp.run();
	}

	function cameraApp() {
	}

	cameraApp.prototype = {
		_pictureSource: null,
		
		_destinationType: null,
    
		run: function() {
			var that = this;
			that._pictureSource = navigator.camera.PictureSourceType;
			that._destinationType = navigator.camera.DestinationType;
			id("capturarFotoClose").addEventListener("click", function() {
				that._capturarFotoClose.apply(that, arguments)
			});
			id("galeriaFotoClose").addEventListener("click", function() {
				that._galeiraFotoClose.apply(that, arguments)
			});
        
			id("capturarFotoCorpo").addEventListener("click", function() {
				that._capturarFotoCorpo.apply(that, arguments)
			});
			id("galeriaFotoCorpo").addEventListener("click", function() {
				that._galeiraFotoCorpo.apply(that, arguments)
			});
		},

		/*Foto Close*/
    
		_capturarFotoClose: function() {
			var that = this;
			// Take picture using device camera, allow edit, and retrieve image as base64-encoded string. 
			// The allowEdit property has no effect on Android devices.
			navigator.camera.getPicture(function() {
				that._onFotoCloseDataSucesso.apply(that, arguments);
			}, function() {
				that._onFalhaFotoClose.apply(that, arguments);
			}, {
				quality: 70, 
				allowEdit: true,
				destinationType: cameraApp._destinationType.DATA_URL,
				targetWidth: 60,
				targetHeight: 60
			});
		},
    
		_galeiraFotoClose: function() {
			var that = this;
			// On Android devices, pictureSource.PHOTOLIBRARY and
			// pictureSource.SAVEDPHOTOALBUM display the same photo album.
			that._buscaFotoCloseGaleria(that._pictureSource.PHOTOLIBRARY);         
		},
    
		_buscaFotoCloseGaleria: function(source) {
			var that = this;
			// Retrieve image file location from specified source.
			navigator.camera.getPicture(function() {
				that._onFotoCloseGaleriaURISucesso.apply(that, arguments);
			}, function() {
				cameraApp._onFalhaFotoClose.apply(that, arguments);
			}, {
				quality: 70,
				destinationType: cameraApp._destinationType.DATA_URL,
				targetWidth: 60,
				targetHeight: 60,
				sourceType: source
			});
		},
    
		_onFotoCloseDataSucesso: function(imageData) {
			var smallImage = document.getElementById('ImgPequena');
			smallImage.style.display = 'block';
			// Show the captured photo.
			smallImage.src = "data:image/jpeg;base64," + imageData;
			viewModel.colaboradorSelecionado.set("ColFoto", smallImage.src);
		},
    
		_onFotoCloseGaleriaURISucesso: function(imageURI) {
			var smallImage = document.getElementById('ImgPequena');
			smallImage.style.display = 'block';
			// Show the captured photo.
			smallImage.src = "data:image/jpeg;base64," + imageURI;
			viewModel.colaboradorSelecionado.set("ColFoto", smallImage.src);
		},
    
		_onFalhaFotoClose: function(message) {
			viewModel.colaboradorSelecionado.set("ColFoto", "data:image/png;base64,");
		},

		/*Foto Corpo*/
		_capturarFotoCorpo: function() {
			var that = this;
			// Take picture using device camera, allow edit, and retrieve image as base64-encoded string. 
			// The allowEdit property has no effect on Android devices.
			navigator.camera.getPicture(function() {
				that._onFotoCorpoDataSucesso.apply(that, arguments);
			}, function() {
				that._onFalhaFotoCorpo.apply(that, arguments);
			}, {
				quality: 70, 
				allowEdit: true,
				destinationType: cameraApp._destinationType.DATA_URL,
				targetWidth: 210,
				targetHeight: 562
			});
		},
    
		_galeiraFotoCorpo: function() {
			var that = this;
			// On Android devices, pictureSource.PHOTOLIBRARY and
			// pictureSource.SAVEDPHOTOALBUM display the same photo album.
			that._buscaFotoCorpoGaleria(that._pictureSource.PHOTOLIBRARY);         
		},
    
		_buscaFotoCorpoGaleria: function(source) {
			var that = this;
			// Retrieve image file location from specified source.
			navigator.camera.getPicture(function() {
				that._onFotoCorpoGaleriaURISucesso.apply(that, arguments);
			}, function() {
				cameraApp._onFalhaFotoCorpo.apply(that, arguments);
			}, {
				quality: 70,
				destinationType: cameraApp._destinationType.DATA_URL,
				targetWidth: 210,
				targetHeight: 562,
				sourceType: source
			});
		},
    
		_onFotoCorpoDataSucesso: function(imageData) {
			var smallImage = document.getElementById('ImgGrande');
			smallImage.style.display = 'block';
			// Show the captured photo.
			smallImage.src = "data:image/jpeg;base64," + imageData;
			viewModel.colaboradorSelecionado.set("ColFotoCorpo", smallImage.src);
		},
    
		_onFotoCorpoGaleriaURISucesso: function(imageURI) {
			var smallImage = document.getElementById('ImgGrande');
			smallImage.style.display = 'block';
			// Show the captured photo.
			smallImage.src = "data:image/jpeg;base64," + imageURI;
			viewModel.colaboradorSelecionado.set("ColFotoCorpo", smallImage.src);
		},
    
		_onFalhaFotoCorpo: function(message) {
			viewModel.colaboradorSelecionado.set("ColFotoCorpo", "data:image/png;base64,");
		}
	}
})(jQuery);